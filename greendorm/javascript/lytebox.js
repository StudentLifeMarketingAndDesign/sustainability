...
LyteBox.prototype.start = function(imageLink, doSlide, doFrame) {
	...
	if (this.isLyteframe) {
		...
		if ((imageLink.getAttribute('rel') == 'lyteframe')) { // add a single frame page to frameArray
			...
			this.frameArray.push(new Array(imageLink.getAttribute('href'), imageLink.getAttribute('alt'), (rev == null || rev == '' ? 'width: 400px; height: 400px; scrolling: auto;' : rev)));
			...
			if (imageLink.getAttribute('rel').indexOf('lyteframe') != -1) {
				for (var i = 0; i < anchors.length; i++) {
					...
					if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == imageLink.getAttribute('rel'))) {
						...
						this.frameArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('alt'), (rev == null || rev == '' ? 'width: 400px; height: 400px; scrolling: auto;' : rev)));
					}
				}
		...
		if ((imageLink.getAttribute('rel') == 'lytebox')) {	// add single image to imageArray
			this.imageArray.push(new Array(imageLink.getAttribute('href'), imageLink.getAttribute('alt')));
			...
			if (imageLink.getAttribute('rel').indexOf('lytebox') != -1) {
				for (var i = 0; i < anchors.length; i++) {
					var anchor = anchors[i];
					if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == imageLink.getAttribute('rel'))) {
						this.imageArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('alt')));
					}
				}
			...
			if (imageLink.getAttribute('rel').indexOf('lyteshow') != -1) {
				for (var i = 0; i < anchors.length; i++) {
					var anchor = anchors[i];
					if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == imageLink.getAttribute('rel'))) {
						this.slideArray.push(new Array(anchor.getAttribute('href'), anchor.getAttribute('alt')));
					}
				}
		...
		for (var i = 0; i < parent.frames.length; i++) {
			...
			try {
				...
				for (var j = 0; j < embeds.length; j++) {
					embeds[j].style.visibility = (state == "hide") ?